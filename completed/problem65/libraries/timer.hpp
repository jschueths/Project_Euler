...
ostream& operator<<(ostream &out, const Timer &data)
{
  ...
  {
    if(data.m_stopped)
      out << setprecision(4) <<  ((double) data.m_end - data.m_start) / CLOCKS_PER_SEC;
    else
      out << setprecision(4) << ((double) clock() - data.m_start) / CLOCKS_PER_SEC;
  }
  else
  {
    out << setprecision(4) << data.m_duration - (((double) clock() - data.m_start) / CLOCKS_PER_SEC);
  }
  return out;
}
